		...
		<div class="tab_list">
			...
			<div class="tabContent">
				<table class="tab_body">
        			<tbody>
        				<!-- ************************************************start********************************************************************* -->
						<c:forEach items="${page.records }" var="pndDetail" varStatus="status">
						<tr>
							<td style="width:4%;" class="td_sel" idv="${pndDetail.id} ">
								<i></i>
							</td>
							<td style="width:2%;">${status.count}</td>
							<td style="width:8%;">${pndDetail.model }</td>
							<td style="width:10%;">${pndDetail.os }</td>
							<td style="width:10%;">${pndDetail.lcd }</td>
							<td style="width:10%;">${pndDetail.cpu }</td>
							
							<td style="width:15%;">${pndDetail.ram }</td>
							<td style="width:15%;">${pndDetail.rom }</td>
							<td style="width:10%;">${pndDetail.dvr }</td>
							
					
							<c:if test="${pndDetail.htmlStatic==0 }">
								<td style="width:4%;">否</td>
							</c:if>
							<c:if test="${pndDetail.htmlStatic==1 }">
								<td style="width:4%;">是</td>
							</c:if>
							<td style="width:12%;" class="td_def_w">
								<div class="tab_action">
									<c:if test="${sessionScope.user.permissions>=1 }">
										<a href="${pageContext.request.contextPath}/servlet/PndControllerServlet?ctrl=show&id=${pndDetail.id}&htmlStatic=${pndDetail.htmlStatic}&model=${pndDetail.model}" title="查看" class="see" target="_blank"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=3 }">
										<a href="${pageContext.request.contextPath}/servlet/PndControllerServlet?ctrl=edit&id=${pndDetail.id}" title="编辑" class="edit"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=7 }">
										<a href="javascript:del(${pndDetail.id})" title="删除" class="del"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=15 }">
										<a href="javascript:saveHmtl('${pndDetail.id}','${pndDetail.model}')" title="静态化" class="save"></a>
									</c:if>								
								</div>
							</td>
						</tr>
		...
		<div class="tool">
			<c:if test="${sessionScope.user.permissions>=15 }">
				...
				<li class="tool_navi_sel">
					<a href="${pageContext.request.contextPath}/addPndDetail.jsp">
			...
			<div class="search">
				<form action="${pageContext.request.contextPath}/servlet/PndControllerServlet?ctrl=search" method="post">
			...
			<div class="pagelist">
				<ul>
					...
					<li><a href="${pageContext.request.contextPath}/pnd.jsp">首页 </a></li>
					<li><a href="${pageContext.request.contextPath}/pnd.jsp?currentPageNum=${page.proviousPageNum}">上一页 </a></li>
					<li><a href="${pageContext.request.contextPath}/pnd.jsp?currentPageNum=${page.nextPageNum}">下一页</a></li>
					<li><a href="${pageContext.request.contextPath}/pnd.jsp?currentPageNum=${page.endPage}">尾面</a> 	</li>
				</ul>
			    <!-- 
			    <p class="pagelist">
...
<script type="text/javascript">
	...
	function del(id){
	 		...
	 		if(sure)
	 			window.location.href = "${pageContext.request.contextPath}/servlet/PndControllerServlet?ctrl=del&id="+id;
	 	}
 	function saveHmtl(id,model){
 		var sure = window.confirm("确定要生成静态页面吗？");
 		if(sure)
 			window.location.href = "${pageContext.request.contextPath}/servlet/PndControllerServlet?ctrl=pageStatic&id="+id+"&fileName="+model;
 	}
	$(function(){
		//表格选种
		...
		$("#delAll").click(function(){
			...
			//创建Form表单
	  		var form = $("<form></form>");
	  		//设置Form表单属性
	  		form.attr({action:"${pageContext.request.contextPath}/servlet/PndControllerServlet?ctrl=delAll",method:"POST"});
	  		//给Form设置值 
