	...
	<style type="text/css">
		*{padding:0px;margin:0px;font-size:12px;font-family: "微软雅黑";}
		...
		/********************tool start ************************************/
		 ...
		 .table .tool #tools li a i.tool_icon1{background-position:       -104px -52px;}
		...
		.tab_list .tab_body tr td .tab_action a.see{background-color: #6297bc;background-position: -334px -134px;}
		...
		.tab_list .tab_body tr td .tab_action a.share{background-color: #28b779;background-position: -291px -47px;}
		...
		<div class="tab_list">
			<table class="tab_head">
				<thead>
					<tr>
						...
						<th style="width:8%;">Manual Name</th>
						<th style="width:8%;">File Size</th>
						<th style="width:8%;">Author</th>
						<th style="width:10%;">DownLoad Link</th>
			...
			<div class="tabContent">
				<table class="tab_body">
        			<tbody>
        				<!-- ************************************************start********************************************************************* -->
						<c:forEach items="${page.records }" var="manual" varStatus="status">
						<c:set var="myManual" value="${manual.objName }"></c:set>
						<tr>
							<td style="width:4%;" class="td_sel" idv="${manual.id}" >
								<i></i>
							</td>
							<td style="width:2%;">${status.count}</td>
							<td style="width:8%;">${manual.productName }</td>
							<td style="width:8%;">${manual.fileSize/(1000*1000) }M</td>
							<td style="width:8%;">${manual.author }</td>
							<td style="width:10%;">
								<a href="${pageContext.request.contextPath}/servlet/DownLoadServlet?category=${manual.objName}&fileType=manual&fileName=${manual.fileName }" class="btn"> 下载说明书</a>
								
							</td>
							<td class="td_def_w"  style="width:12%;">
								<div class="tab_action">
									<c:if test="${sessionScope.user.permissions>=1 }">
										<a href="${pageContext.request.contextPath}/servlet/DownLoadServlet?category=${manual.objName}&fileType=manual&fileName=${manual.fileName }" title="下载" class="see"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=3 }">
										<a href="${pageContext.request.contextPath}/servlet/ManualControllerServlet?ctrl=edit&id=${manual.id}&category=${manual.objName}" title="编辑" class="edit"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=7 }">
										<a href="javascript:del(${manual.id},'${manual.objName}')" title="删除" class="del"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=15 }">
										<a href="javascript:share('${manual.id}','${manual.objName}')" title="分享" class="share"></a>
									</c:if>
								</div>
							</td>
						</tr>
		...
		<div class="tool">
			...
			<ul id="tools">
				<li class="tool_navi_sel">
					<a href="${pageContext.request.contextPath}/upLoadManual.jsp">
			...
			<div class="search">
				<form action="${pageContext.request.contextPath}/servlet/ManualControllerServlet?ctrl=search&category=${myManual}" method="post">
			...
			<div class="pagelist">
				<ul>
					...
					<li><a href="${pageContext.request.contextPath}/${fn:toLowerCase(myManual)}Manual.jsp">首页 </a></li>
					<li><a href="${pageContext.request.contextPath}/${fn:toLowerCase(myManual)}Manual.jsp?currentPageNum=${page.proviousPageNum}">上一页 </a></li>
					<li><a href="${pageContext.request.contextPath}/${fn:toLowerCase(myManual)}Manual.jsp?currentPageNum=${page.nextPageNum}">下一页</a></li>
					<li><a href="${pageContext.request.contextPath}/${fn:toLowerCase(myManual)}Manual.jsp?currentPageNum=${page.endPage}">尾面</a> 	</li>
				</ul>
			    <!-- 
			    <p class="pagelist">
...
<script type="text/javascript">
	...
	function del(id,category){
	 		...
	 		if(sure)
	 			window.location.href = "${pageContext.request.contextPath}/servlet/ManualControllerServlet?ctrl=del&id="+id+"&category="+category;
	 	}
 	function share(id,model){
 		/* var sure = window.confirm("确定要生成静态页面吗？");
 		if(sure)
 			window.location.href = "${pageContext.request.contextPath}/servlet/ManualControllerServlet?ctrl=modify&id="+id+"category"+category;
 	 	*/
 	 	
 	 	window.alert("分享正在赶来的路上，敬请期待！");
 	}
	$(function(){
		//表格选种
		...
		$("#delAll").click(function(){
			...
			if($(".check_on").length<1){
				alert("4不4撒，你什么都没有选中！${manual.objName}");
				return false;
			}
			//创建Form表单
	  		var form = $("<form></form>");
	  		//设置Form表单属性
	  		form.attr({action:"${pageContext.request.contextPath}/servlet/ManualControllerServlet?ctrl=delAll&category=${myManual}",method:"POST"});
	  		//给Form设置值 
