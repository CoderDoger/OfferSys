...
public class HtmlStatic{
	...
	String line = null;   
		//构造方法  
		public HtmlStatic(String webappname){  
		...
		public boolean createHtmlPage(String url,String htmlFileName){  
		    ...
		    try{  
		        ...
		        if (statusCode!=200) {  
		        	throw new RuntimeException("静态页面引擎在解析"+url+"产生静态页面"+htmlFileName+"时出错!");  
		        }else{  
		            //读取解析结果  
		            sb = new StringBuffer();  
		        ...
		        throw new RuntimeException("静态页面引擎在解析"+url+"产生静态页面"+htmlFileName+"时出错:"+ex.getMessage());           
		    }finally{  
		        //释放http连接  
	    ...
	    public static void main(String[] args){  
		 	HtmlStatic h = new HtmlStatic("OfferSys"); 
	        h.createHtmlPage("http://localhost/OfferSys/servlet/ControllerServlet?ctrl=show&id=2","c:/test.html");  
