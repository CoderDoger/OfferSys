		...
		<div class="tab_list">
			<table class="tab_head">
				<thead>
					<tr>
						...
						<th  style="width:10%;">FLASH</th>
						<th  style="width:10%;">DVR</th>
						<th  style="width:10%;">Charger</th>
			...
			<div class="tabContent">
				<table class="tab_body">
        			<tbody>
        				<!-- ************************************************start********************************************************************* -->
						<c:forEach items="${page.records }" var="mirrorDetail" varStatus="status">
						<tr>
							<td style="width:4%;" class="td_sel" idv="${mirrorDetail.id} ">
								<i></i>
							</td>
							<td style="width:2%;">${status.count}</td>
							<td style="width:8%;">${mirrorDetail.model }</td>
							<td style="width:10%;">${mirrorDetail.os }</td>
							<td style="width:10%;">${mirrorDetail.lcd }</td>
							<!-- 
							<td>${gpsDetail.tp }</td> 
							<td>${gpsDetail.cpu }</td>
							<td>${gpsDetail.bluetooth }</td>
							<td>${gpsDetail.fmt }</td>
							<td>${gpsDetail.avin }</td>
							<td>${gpsDetail.tv }</td>
							-->
							<td style="width:10%;">${mirrorDetail.ram }</td>
							<td style="width:10%;">${mirrorDetail.rom }</td>
							<td style="width:10%;">${mirrorDetail.dvr }</td>
							<td style="width:10%;">${mirrorDetail.charger }</td>
							<td style="width:10%;">${mirrorDetail.battery }</td>
					
							<c:if test="${mirrorDetail.htmlStatic==0 }">
								<td style="width:4%;">否</td>
							</c:if>
							<c:if test="${mirrorDetail.htmlStatic==1 }">
								<td style="width:4%;">是</td>
							</c:if>
							<td style="width:12%;" class="td_def_w">
								<div class="tab_action">
									<c:if test="${sessionScope.user.permissions>=1 }">
										<a href="${pageContext.request.contextPath}/servlet/MirrorControllerServlet?ctrl=show&id=${mirrorDetail.id}&htmlStatic=${mirrorDetail.htmlStatic}&model=${mirrorDetail.model}" title="查看" class="see" target="_blank"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=3 }">
										<a href="${pageContext.request.contextPath}/servlet/MirrorControllerServlet?ctrl=edit&id=${mirrorDetail.id}" title="编辑" class="edit"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=7 }">
										<a href="javascript:del(${mirrorDetail.id})" title="删除" class="del"></a>
									</c:if>
									<c:if test="${sessionScope.user.permissions>=15 }">
										<a href="javascript:saveHmtl('${mirrorDetail.id}','${mirrorDetail.model}')" title="静态化" class="save"></a>
									</c:if>								
								</div>
							</td>
						</tr>
		...
		<div class="tool">
			<c:if test="${sessionScope.user.permissions>=15 }">
				...
				<li class="tool_navi_sel">
					<a href="${pageContext.request.contextPath}/addMirrorDetail.jsp">
			...
			<div class="search">
				<form action="${pageContext.request.contextPath}/servlet/MirrorControllerServlet?ctrl=search" method="post">
			...
			<div class="pagelist">
				<ul>
					...
					<li><a href="${pageContext.request.contextPath}/mirror.jsp">首页 </a></li>
					<li><a href="${pageContext.request.contextPath}/mirror.jsp?currentPageNum=${page.proviousPageNum}">上一页 </a></li>
					<li><a href="${pageContext.request.contextPath}/mirror.jsp?currentPageNum=${page.nextPageNum}">下一页</a></li>
					<li><a href="${pageContext.request.contextPath}/mirror.jsp?currentPageNum=${page.endPage}">尾面</a> 	</li>
				</ul>
			    <!-- 
			    <p class="pagelist">
...
<script type="text/javascript">
	...
	function del(id){
 		...
 		if(sure)
 			window.location.href = "${pageContext.request.contextPath}/servlet/MirrorControllerServlet?ctrl=del&id="+id;
 	}
 	function saveHmtl(id,model){
 		var sure = window.confirm("确定要生成静态页面吗？");
 		if(sure)
 			window.location.href = "${pageContext.request.contextPath}/servlet/MirrorControllerServlet?ctrl=pageStatic&id="+id+"&fileName="+model;
 	}
	$(function(){
		//表格选种
		...
		$("#delAll").click(function(){
			...
			//创建Form表单
	  		var form = $("<form></form>");
	  		//设置Form表单属性
	  		form.attr({action:"${pageContext.request.contextPath}/servlet/MirrorControllerServlet?ctrl=delAll",method:"POST"});
	  		//给Form设置值 
